...
def main(input_saved_model_dir_path, input_classes_path, input_image_dir_path, answer_image_dir_path, output_json_dir_path):
    ...
    for image_path, output_elem in zip(image_path_list, output):
        answer_image = np.asarray(Image.open(os.path.join(answer_image_dir_path, os.path.basename(image_path).replace('.', '_seg.'))).convert('L'))
        output_json_path = os.path.join(output_json_dir_path, os.path.splitext(os.path.basename(image_path))[0]+'.json')
        output_elem['answer'] = answer_image.flatten().tolist()
        output_elem['labels'] = output_elem['labels'].flatten().tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_saved_model_dir_path', type=str, default='~/output_model/2023-03-04-19-07-35/step-5000_batch-8_epoch-3_loss_0.0017_one_hot_mean_io_u_0.8605_val_loss_0.1085_val_one_hot_mean_io_u_0.1840')
    parser.add_argument('--input_classes_path', type=str, default=os.path.join(os.path.dirname(__file__), 'test_images/classes.txt'))
    parser.add_argument('--input_image_dir_path', type=str, default=os.path.join(os.path.dirname(__file__), 'test_images/raw'))
    parser.add_argument('--answer_image_dir_path', type=str, default=os.path.join(os.path.dirname(__file__), 'test_images/seg'))
    parser.add_argument('--output_json_dir_path', type=str, default='~/.vaik-mnist-segmentation-dataset/test_images_out')
